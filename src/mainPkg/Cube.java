...
public class Cube {
	...
	public static Cube makeStep8Test() {
		int[][] front = { { GREEN, ORANGE, GREEN }, { GREEN, GREEN, GREEN }, { GREEN, GREEN, GREEN } };
		int[][] back = { { BLUE, BLUE, BLUE }, { BLUE, BLUE, BLUE }, { BLUE, BLUE, BLUE } };
		int[][] left = { { RED, GREEN, RED }, { RED, RED, RED }, { RED, RED, RED } };
		int[][] right = { { ORANGE, RED, ORANGE }, { ORANGE, ORANGE, ORANGE }, { ORANGE, ORANGE, ORANGE } };
		int[][] up = { { YELLOW, YELLOW, YELLOW }, { YELLOW, YELLOW, YELLOW }, { YELLOW, YELLOW, YELLOW } };
		int[][] down = { { WHITE, WHITE, WHITE }, { WHITE, WHITE, WHITE }, { WHITE, WHITE, WHITE } };
	...
	public static Cube makeStep7Test() {
		int[][] front = { { GREEN, GREEN, ORANGE }, { GREEN, GREEN, GREEN }, { GREEN, GREEN, GREEN } };
		int[][] back = { { RED, BLUE, GREEN }, { BLUE, BLUE, BLUE }, { BLUE, BLUE, BLUE } };
		int[][] left = { { ORANGE, RED, RED }, { RED, RED, RED }, { RED, RED, RED } };
		int[][] right = { { BLUE, ORANGE, BLUE }, { ORANGE, ORANGE, ORANGE }, { ORANGE, ORANGE, ORANGE } };
		int[][] up = { { YELLOW, YELLOW, YELLOW }, { YELLOW, YELLOW, YELLOW }, { YELLOW, YELLOW, YELLOW } };
		int[][] down = { { WHITE, WHITE, WHITE }, { WHITE, WHITE, WHITE }, { WHITE, WHITE, WHITE } };
	...
	public static int opFace(int face) {
		if (face == Cube.FRONT) {
			...
			return Cube.LEFT;
		} else if (face == Cube.UP) {
			return Cube.DOWN;
		} else if (face == Cube.DOWN) {
			return Cube.UP;
		} else {
			throw new IllegalArgumentException("Invalid face index. Index must be between 0 and 5. Given: " + face);
		}
	}
	
	/**
	 * Determine the color that should be opposite a face with the given color
	 * @param color
	 * @return The int representing the opposing color
	 */
	public static int opColor(int color) {
		switch(color) {
		case Cube.WHITE:
			return Cube.YELLOW;
		case Cube.YELLOW:
		...
		case Cube.ORANGE:
			return Cube.RED;
		default:
			throw new IllegalArgumentException("Invalid color. Color index must be between 0 and 5. Given: " + color);
		}
	}
	
	/**
	 * Determine if the cube is a valid Rubik's cube, that is, it has 9 each of 
	 * 6 different colored tiles, the blue center is opposite green, red is
	 * opposite orange, and yellow is opposite white. Does not check for solvability.
	 * @return True if the cube is valid, false otherwise
	 */
	public boolean isValid() {
		// Keep track of how many of each color we've seen
		int[] colorAccum = new int[6];
		
		for (int face = 0; face < 6; face++) {
			for (int row = 0; row < 3; row++) {
				for (int col = 0; col < 3; col++) {
					if (row == 1 && col == 1) {
						// Center tile, check opposite center
						if (data[face][row][col] != opColor(data[opFace(face)][row][col])) {
							return false;
						}
					}
					
					colorAccum[data[face][row][col]] += 1;
				}
			}
		}
		
		for (int count : colorAccum) {
			if (count != 9) {
				return false;
			}
		}
	...
	public boolean equals(Object other) {
		if (other == null || !(other instanceof Cube)) {
			...
			for (int face = 0; face < 6; face++) {
				for (int row = 0; row < 3; row++) {
					for (int col = 0; col < 3; col++) {
						if (data[face][row][col] != otherCube.data[face][row][col]) return false;
					}
				}
			}
	...
	public String toString() {
		...
		for (int row = 0; row < 3; ++row) {
			for (int col = 0; col < 3; ++col) {
				out += this.left[row][col];
				out += " ";
			}
			out += "  ";
			for (int col = 0; col < 3; ++col) {
				out += this.front[row][col];
				out += " ";
			}
			out += "  ";
			for (int col = 0; col < 3; ++col) {
				out += this.right[row][col];
				out += " ";
			}
			out += "\n";
		}
