...
public class Cube {
	...
	public static final int NOT_GREEN = 6;
	public static final int NOT_ORANGE = 7;
	public static final int NOT_BLUE = 8;
	public static final int NOT_RED = 9;
	public static final int NOT_YELLOW = 10;
	public static final int NOT_WHITE = 11;
	...
	public static final int[] SIDES = {FRONT, RIGHT, BACK, LEFT};
	public static final int[] FACES = {FRONT, RIGHT, BACK, LEFT, UP, DOWN};
	...
	public Cube(int[][] front, int[][] right, int[][] back, int[][] left, int[][] up, int[][] down) {
		this.front = front;
		this.right = right;
		this.back = back;
		this.left = left;
		...
		int[][][] myData = {this.front, this.right, this.back, this.left, this.up, this.down};
	...
	public int[][][] sides(){
		int[][][] sides = {this.front, this.back, this.left, this.right};
		return sides;
	}
	...
	// returns an int[] with the colors adjacent to a face
	// WARNING: face must be front, right, back, or left
	public int[] adjacentColors(int row, int col, int face){
		
		if (row == 0 && col == 0){
			int[] result = {this.data[Main.leftFace(face)][0][2], upFrom(2, 0, face)};
			return result;
		} else if (row == 0 && col == 1){
			int [] result = {upFrom(2,1,face)};
			return result;
		} else if (row == 0 && col == 2){
			int [] result = {this.data[Main.rightFace(face)][0][0], upFrom(2, 2, face)};
			return result;
		} else if (row == 1 && col == 0){
			int[] result = {this.data[Main.leftFace(face)][1][2]};
			return result;
		} else if (row == 1 && col == 2){
			int[] result = {this.data[Main.rightFace(face)][1][0]};
			return result;
		} else if (row == 2 && col == 0){
			int[] result = {this.data[Main.leftFace(face)][2][2], downFrom(0, 0, face)};
			return result;
		} else if (row == 2 && col == 1){
			int[] result = {downFrom(0, 1, face)};
			return result;
		} else {
			int[] result = {this.data[Main.rightFace(face)][2][0], downFrom(0, 2, face)};
			return result;
		}
	...
	private void turnFront(int times){
		
		for(int i = 0; i < times; ++i){
			this.turnFrontHelp();
		}
		this.steps.add(new Step(Cube.FRONT, times));
	}
	
	private void turnBack(int times){
		
		for(int i = 0; i < times; ++i){
			this.turnBackHelp();
		}
		this.steps.add(new Step(Cube.BACK, times));
	}
		
	private void turnRight(int times){
		
		for(int i = 0; i < times; ++i){
			this.turnRightHelp();
		}
		this.steps.add(new Step(Cube.RIGHT, times));
	}
	
	private void turnLeft(int times){
		
		for(int i = 0; i < times; ++i){
			this.turnLeftHelp();
		}
		this.steps.add(new Step(Cube.LEFT, times));	
	}
	...
	public void turnUp(int times){
		...
		this.steps.add(new Step(Cube.UP, times));
	}
	...
	public void turnDown(int times){
		...
		this.steps.add(new Step(Cube.DOWN, times));
	}
	...
	// only works if the input face is front, back, left, or right
	...
	public int downFrom(int row, int col, int face){
		if(face == FRONT){
			return this.down[row][col];
		} else if (face == RIGHT){
			return this.down[2-col][row];
		} else if (face == BACK){
			return this.down[2-row][2-col];
		} else {
			return this.down[col][2-row];
		}
	}
}
