...
public class CubeParserTest {
	...
	/**
	 * With 6 images of the six faces of a real cube, test the
	 * full parsing pipeline. Run ImageParser on these images
	 * to get an array of FaceColors, and then run CubeParser
	 * on the result to get a valid cube.
	 */
	public static String testCube1_fullParse() {
		...
		Cube correctCube = new Cube(correctArray[0], correctArray[1], correctArray[2], correctArray[3], correctArray[4], correctArray[5]);
		if(!correctCube.isValid()) return "Unable to run test: testCube1_fullParse. Given invalid correct cube:\n" + correctCube;
		...
		if (!test.isValid()) return "Failed test: testCube1_fullParse. Invalid resulting cube:\n" + test;
		else if (!test.equals(correctCube)) return "Failed test: testCube1_fullParse.\n\tExpected cube:\n" + correctCube + "\n\tActual cube:\n" + test;
		else return "Passed test: testCube1_fullParse.";
	}
}
